...
scenarios.forEach(scenario => {
  ...
  operations.forEach(op => {
    ...
    const improvement = Math.abs(((pgAvg - mongoAvg) / Math.max(pgAvg, mongoAvg)) * 100).toFixed(1);
    
    const opName = op.replace(/([A-Z])/g, ' $1').trim();
    console.log(`│ ${opName.padEnd(15)} │ ${pgAvg.toString().padEnd(12)} │ ${mongoAvg.toString().padEnd(12)} │ ${winner.padEnd(10)} │`);
  });
...
operations.forEach(op => {
  ...
  const opName = op.replace(/([A-Z])/g, ' $1').trim();
  const improvement = ((Math.max(pgStats.avgTime, mongoStats.avgTime) - Math.min(pgStats.avgTime, mongoStats.avgTime)) / Math.max(pgStats.avgTime, mongoStats.avgTime) * 100).toFixed(1);
  const faster = pgStats.avgTime < mongoStats.avgTime ? 'PostgreSQL' : 'MongoDB';
  
  console.log(chalk.cyan(`${opName.toUpperCase()}:`));
  console.log(`  PostgreSQL: Avg ${pgStats.avgTime}ms | P99 ${pgStats.p99}ms | Success ${pgStats.successRate}%`);
  console.log(`  MongoDB:    Avg ${mongoStats.avgTime}ms | P99 ${mongoStats.p99}ms | Success ${mongoStats.successRate}%`);
