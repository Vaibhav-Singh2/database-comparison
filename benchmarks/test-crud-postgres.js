...
async function testRead(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const result = await measureOperation(() =>
      axios.get(`${BASE_URL}/api/pg/users/${userId}`)
    );
    results.push(result);
...
async function testWrite(count) {
  ...
  for (let i = 0; i < count; i++) {
    const result = await measureOperation(() =>
      axios.post(`${BASE_URL}/api/pg/users`, {
...
async function testComplexQuery(count) {
  ...
  for (let i = 0; i < count; i++) {
    const userId = Math.floor(Math.random() * 1000) + 1;
    const result = await measureOperation(() =>
      axios.get(`${BASE_URL}/api/pg/users/${userId}/orders`)
    );
    results.push(result);
...
async function testProductQuery(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const result = await measureOperation(() =>
      axios.get(`${BASE_URL}/api/pg/products?category=${category}&limit=20`)
    );
    results.push(result);
...
async function runTests() {
  ...
  for (let i = 0; i < 60; i++) {
    try {
      ...
      const testQuery = await axios.get(`${BASE_URL}/api/pg/users/1`, {
        timeout: 2000,
      });
  ...
  if (!serverReady) {
    ...
    console.log(
      chalk.yellow("  Make sure databases are seeded: npm run seed-all\n")
    );
    process.exit(1);
  }
  console.log("");
  ...
  for (const scenario of TEST_SCENARIOS) {
    ...
    console.log(
      chalk.yellow(
        `  ${scenario.name} Load (${scenario.operations} operations)`
      )
    );
    ...
    const complexStats = await testComplexQuery(
      Math.floor(scenario.operations * 0.3)
    ); // 30% complex
    const productStats = await testProductQuery(
      Math.floor(scenario.operations * 0.5)
    ); // 50% product queries
    ...
    if (writeStats) {
      console.log(
        chalk.green(
          `  ✓ WRITE: Avg ${writeStats.avgTime}ms | P99 ${writeStats.p99}ms`
        )
      );
    } else {
    ...
    if (complexStats) {
      console.log(
        chalk.green(
          `  ✓ COMPLEX: Avg ${complexStats.avgTime}ms | P99 ${complexStats.p99}ms`
        )
      );
    } else {
    ...
    if (productStats) {
      console.log(
        chalk.green(
          `  ✓ PRODUCT: Avg ${productStats.avgTime}ms | P99 ${productStats.p99}ms`
        )
      );
    } else {
  ...
  console.log(chalk.green("\n✅ PostgreSQL benchmarks complete!"));
  console.log(
    chalk.cyan(`   Results saved to: benchmarks/postgres-results.json\n`)
  );
}
